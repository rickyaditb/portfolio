...
{#key isInView}
    ...
    <div in:fly={Configuration['animation']['in-fly']} class="px-5 mt-3 flex flex-col gap-2">
        {#each Data.certification as cert, index}
            <article class="flex gap-3 items-center hover:scale-102 transition group">
                ...
                <div class="ml-auto items-center gap-3 opacity-0 group-hover:opacity-100 transition duration-500 ease-in-out hidden lg:flex">
                    <button class="border-2 border-white p-2 rounded flex items-center gap-2" on:click={() => copyToClipboard(cert.credential, index)}>
                    ...
                    <a href={cert.verification} class="flex items-center gap-2 font-bold bg-secondary py-2 px-3 rounded">
